...
class SettingsView(ttk.Toplevel):
    ...
    def _configure_components(self) -> None:
        ...
        self._floating_masked_heatmap_postfix_label.configure(text="Postfix of Floating waste masked heatmap image:")
    ...
    def _place_components(self) -> None:
        ...
        self._c_id_frame.grid(row=3, column=2, columnspan=2, sticky="nsew", padx=5, pady=5)
    ...
    def _create_tooltips(self) -> None:
        ...
        ToolTip(
            ...
            "The larger the number, the more unique pixels are visible on result images.",
        )
        ...
        ToolTip(
            ...
            "The Class ID of the Garbage class in the loaded Random Forest classifiers.",
        )
        ToolTip(
            self._water_c_id_label,
            "The Class ID of the Water class in the loaded Random Forest classifiers.",
        )

        # Path settings
        ToolTip(
            self._working_dir_label,
            "The working directory of the application. The result images will be saved here.",
        )
        ToolTip(
            self._hotspot_rf_label,
            "The path of the saved Random Forest classifier model for the Hot-spot detection method.",
        )
        ToolTip(
            self._floating_rf_label,
            "The path of the saved Random Forest classifier model for the Floating waste detection method.",
        )
        ToolTip(self._working_dir_browse_btn, "Browse working directory.")
        ToolTip(
            self._hotspot_rf_browse_btn,
            "Browse saved Random Forest classifier model for the Hot-spot detection method.",
        )
        ToolTip(
        ...
        ToolTip(
            ...
            "File name postfix of the masked and classified images " "(Floating waste detection).",
        )
        ToolTip(
            self._floating_masked_heatmap_postfix_label,
            "File name postfix of the masked heatmap images " "(Floating waste detection).",
        )
        ToolTip(
            self._washed_up_before_postfix_label,
            "File name postfix of the first result image (Washed up waste detection).",
        )
        ToolTip(
        ...
        ToolTip(
            ...
            "Reversed Normalized Difference Vegetation index: (Red - NIR) / (Red + NIR).",
        )

...
class ConvolutionBlock(nn.Module):
    """
    Class that is the building block of the UNET and UNET++ neural network architectures.
    """

    def __init__(self, in_channels, out_channels):
        """
        The constructor of the ConvolutionBlock class.
        """
        super().__init__()
    ...
    def forward(self, x):
        """
        Defines the forward pass of the ConvolutionBlock.
...
class EncoderBlock(nn.Module):
    """
    Class for the Encoder part of the UNET and UNET++ architectures.
    """

    def __init__(self, in_channels, out_channels):
        """
        The construcor of the EncoderBlock class.
        """
        super().__init__()
    ...
    def forward(self, x):
        ...
        :return: The output of the ConvolutionBlock,
        and the output of the ConvolutionBlock passed through a Max-Pooling.
        """
        skip = self.conv(x)
        out = self.mpool(skip)
        return skip, out
...
def center_crop(layer, target_height, target_width):
    """
    Cuts a given tensor to the specified size.

    :param layer: The input tensor.
...
class DecoderBlock(nn.Module):
    """
    Class for the Decoder part of the UNET architecture.
    """

    def __init__(self, in_channels, out_channels):
        """
        The constructor of the DecoderBlock class.
        """
        super().__init__()
    ...
    def forward(self, x, skip):
        """
        Defines the forward pass of the DecoderBlock.

        :param x: the input tensor of shape (batch_size, in_channels, height, width).

        :return: The output of the DecoderBlock.
        """
        out = self.up_conv(x)
...
class UNET(nn.Module):
    """
    Class defining the UNET architecture described in the paper: https://arxiv.org/pdf/1505.04597.
    """
    ...
    def forward(self, x):
        """
        Defines the forward pass of the UNET class.

        :param x: the input tensor of shape (batch_size, in_channels, height, width).

        :return: The output of the UNET class.
        """
        skip1, out = self.e1(x)
    ...
    def predict(self, x):
        """
        Generates predictions using the UNET class.
        First the input tensor is passed through the layers,
        then applies the appropriate activation function, based on the number of output channels.
    ...
    def load(self, path):
        """
        Loads and sets the parameters to a specified state.

        :param path: The path of the state_dict.
        """
        try:
...
class UpConv(nn.Module):
    """
    Class for upconvolution, also applying dropout.
    """

    def __init__(self, in_channels, out_channels):
        """
        The constructor of the UpConv class
        """
        super().__init__()
    ...
    def forward(self, x):
        """
        Defines the forward pass for the UpConv class.
        """
...
class UNETPP(nn.Module):
    """
    The class defining the UNET++ architecture,
    described in the paper: https://arxiv.org/pdf/1807.10165
    """
    ...
    def __init__(
        ...
        """
        The constructor of the UNETPP class.

        :param in_channels: Number of input channels.
        :param out_channels: Number of output channels.
        :param pretrained_unet: It is possible the use
        an existing UNET model as the backbone of the UNET++ architecture.
        :param freeze_weights: If True, the weights of the pretrained UNET
        will be frozen and won't change during training.
        :param deep_vision: If True, the model will use deep supervision described in the paper.
        """
        super().__init__()
    ...
    def align_layers(self, layers):
        """
        This functions cuts and concatenates the inputs together.

        :param layers: List of tensors

        :return: The concatenated tensors, where each original tensor is resized to the smallest.
        """
        layer_heights = []
        layer_widths = []
        rtn = []
    ...
    def predict(self, x):
        """
        Generates predictions using the UNETPP class.
        First the input tensor is passed through the layers,
        then applies the appropriate activation function, based on the number of output channels.
        ...
        x0_1 = self.conv0_1(self.align_layers([skip0_0, self.up_conv1_0(skip1_0)]))
        x1_1 = self.conv1_1(self.align_layers([skip1_0, self.up_conv2_0(skip2_0)]))
        x0_2 = self.conv0_2(self.align_layers([skip0_0, x0_1, self.up_conv1_1(x1_1)]))
        x2_1 = self.conv2_1(self.align_layers([skip2_0, self.up_conv3_0(skip3_0)]))
        x1_2 = self.conv1_2(self.align_layers([skip1_0, x1_1, self.up_conv2_1(x2_1)]))
        x0_3 = self.conv0_3(self.align_layers([skip0_0, x0_1, x0_2, self.up_conv1_2(x1_2)]))
        x3_1 = self.conv3_1(self.align_layers([skip3_0, self.up_conv4_0(skip4_0)]))
        x2_2 = self.conv2_2(self.align_layers([skip2_0, x2_1, self.up_conv3_1(x3_1)]))
        x1_3 = self.conv1_3(self.align_layers([skip1_0, x1_1, x1_2, self.up_conv2_2(x2_2)]))
        x0_4 = self.conv0_4(self.align_layers([skip0_0, x0_1, x0_2, x0_3, self.up_conv1_3(x1_3)]))

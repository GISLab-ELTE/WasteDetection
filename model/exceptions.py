...
class InvalidClassifiedImageException(Exception):
    ...
    def __init__(self, input_path: str):
        message = "Not all values can be divided by 100 on the image! The image: " + input_path
...
class TooLargeImageException(Exception):
    ...
    def __init__(self, actual_count: float, max_count: float, input_path: str):
        message = str(actual_count) + " pixels -> Too large image! The limit is " + str(max_count) + \
                  " pixels. The image: " + input_path
...
class CodValueNotPresentException(Exception):
    ...
    def __init__(self, cod_type: str, cod_value: int, input_path: str):
        message = cod_type.upper() + "_COD value (" + str(cod_value) + ") is not on the image! Wrong "\
                  + cod_type.upper() + " Class ID! The image: " + input_path
...
class NotEnoughBandsException(Exception):
    ...
    def __init__(self, available_bands: int, needed_bands: int, input_path: str):
        message = "Available bands: " + str(available_bands) + " -> " + str(needed_bands) +\
                  " bands needed! The image: " + input_path
...
class FileExtensionException(Exception):
    ...
    def __init__(self, wanted_file_extension: str, got_file_extension: str, input_path: str):
        message = got_file_extension + " -> File extension must be " + wanted_file_extension +\
                  ". The file: " + input_path
